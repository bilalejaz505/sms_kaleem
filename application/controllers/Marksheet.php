...
class Marksheet extends MY_Controller
{
    ...
    */
    ...
    {
        $this->session->unset_userdata('marksheet_id');
        ...
        if ($this->form_validation->run() === true) {
            $this->session->set_userdata(array('marksheet_id' => $this->input->post('marksheetName')));
            $validator['success'] = true;
            $validator['messages'] = "Successfully added";
                ...
                $button = '<div class="btn-group">
				  ...
				  <ul class="dropdown-menu">			  	
				    ...
				    <li><a href="#" data-toggle="modal" data-target="#viewMarksModal" onclick="viewMarks(' . $value['student_id'] . ',' . $classId . ')">View</a></li>	
				     <li><a href="' . base_url() . 'marksheet/printMarkSheet/' . $value['student_id'] . '/' . $classId . '/' . $this->session->userdata('marksheet_id') . '" target="_blank">Print</a></li>	
				  </ul>
				</div>';
    ...
    */
    ...
    {
        $this->session->unset_userdata('section_name');
        if ($classId && $sectionId) {
            ...
            foreach ($studentData as $key => $value) {
                ...
                $classData = $this->model_classes->fetchClassData($value['class_id']);
                $sectionData = $this->model_section->fetchSectionByClassSection($value['class_id'], $value['section_id']);
                $this->session->set_userdata(array('section_name' => $sectionData['section_name']));
    ...
    */
    ...
    public function printMarkSheet($studentId = null, $classId = null, $marksheetId = null)
    {
        if ($studentId && $classId && $marksheetId) {
            $studentMarkData = $this->model_marksheet->viewStudentMarksheet($studentId, $classId, $marksheetId);
            $resultDataArr['student_info'] = $this->model_student->fetchStudentData($studentId);
            $resultDataArr['class_info'] = $this->model_classes->fetchClassData($classId);
            $resultDataArr['class_info']['section'] = $this->session->userdata('section_name');
            $totalMark = 0;
            $obtainMark = 0;

            foreach ($studentMarkData as $key => $value) {
                $subjectData = $this->model_subject->fetchSubjectByClassSection($value['class_id'], $value['subject_id']);
                $resultDataArr['subject_name'][] = $subjectData['name'];
                $resultDataArr['subject_total_marks'][] = 100;
                $resultDataArr['subject_obtained_marks'][] = $value['obtain_mark'];
            ...
            $resultDataArr['total_marks'] = $totalMark;
            $resultDataArr['total_obtained_marks'] = $obtainMark;
            $resultDataArr['percentage'] = $percentage;
            //echo $div;
            //echo '<pre>';print_r($resultDataArr);exit();
            $data = array();
            $html = $this->load->view('templates/marksheet', $resultDataArr, true);
            //echo $html;exit();
    ...
    public function generatePDF($html, $type='open')
    {
        $filename = rand() . ".pdf";
        $mpdf = new mPDF('c', 'A4');
        $mpdf->WriteHTML($html);
